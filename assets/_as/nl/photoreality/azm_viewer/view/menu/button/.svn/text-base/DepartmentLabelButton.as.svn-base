...
package nl.photoreality.azm_viewer.view.menu.button 
{
	...
	public class DepartmentLabelButton extends MovieClip {
		...
		public function init(routeVO:RouteVO,excludePostFixPattern):void {
			_routeVO = routeVO;
			
			label.text = _routeVO.name;
			if (excludePostFixPattern) {
				label.text = _routeVO.name.substr(0, -2);
				if (_menuManager.route){
					if (_menuManager.route.name.substr(0, -2) == label.text) {
