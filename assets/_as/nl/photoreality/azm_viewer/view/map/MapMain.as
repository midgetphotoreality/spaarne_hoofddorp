...
package nl.photoreality.azm_viewer.view.map {
	...
	public class MapMain extends MovieClip {
		...
		public function MapMain()  {
			...
			bg.visible = false;
			label_container_mc.visible = false;
		}
		...
		private function addedToStageListener(e:Event):void {
			...
			route_container_mc.floor2OffsetY = (floor2_container_mc.y - route_container_mc.y);// +77;
			trace("ROUTECONTAINER.X: " + route_container_mc.x);
			trace("ROUTECONTAINER.Y: " + route_container_mc.y);
			//350
			//trace("route_container_mc.floor2OffsetX: "+route_container_mc.floor2OffsetX)
			//trace("route_container_mc.floor2OffsetY: "+route_container_mc.floor2OffsetY)
			
			//label_container_mc.offSet = new Point(route_container_mc.floor2OffsetX, route_container_mc.floor2OffsetY);
		...
		private function init():void {
			...
			var offsetY:int = floor2_container_mc.y - floor1_container_mc.y;// +77;
		...
		private function refresh():void {
			...
			trace("1"+_menuManager.route.floors.length);
				route_container_mc.refresh();trace("1"+_menuManager.route.floors.length);
				addFloor1(_menuManager.route.floors[0]);trace("1");
				
				// check if end floor is different
				if (_menuManager.route.floors[0] != _menuManager.route.floors[1]) {
					addFloor2(_menuManager.route.floors[1]);trace("1a");
				} else {
					floor2_container_mc.deinit();trace("2");
					floor2_name.text = "";trace("2");
				}
