//nl.photoreality.map_editor.view.overlays.OverlayAddFloor
package nl.photoreality.map_editor.view.overlays {
	...
	public class OverlayAddFloor extends Provider {
		...
		public function OverlayAddFloor()  {
		...
		private function submit():void {
			if ( isValidAssetMc(txt_linkage.text) ) {
				Controller.getInstance().closeOverlay();
				FloorController.getInstance().addFloor(new FloorVO(txt_name.text, txt_linkage.text, false, new Vector.<IconVO>));
			}else {
