	...
	var _buildDesktopMenu = function () {
		...
		for (i = 0; i < _currentMenu.length; i++) {
			...
			if (_menus[routeLetter]) {
				div = document.createElement('div');
				div.innerHTML = _currentMenu[i].name;
		...
		/*
		//console.log(_menus)
		for(var letterMenuItem in _menus){
			//console.log(_menus[letterMenuItem].id);
			//console.log(_menus[letterMenuItem].container.getBoundingClientRect());
			////console.log(_menus[letterMenuItem].container.getBoundingClientRect().top)
			////console.log(_menus[letterMenuItem].container.scrollHeight)
			////console.log(_menus[letterMenuItem].container.getBoundingClientRect().top + _menus[letterMenuItem].container.scrollHeight);
			
			
			if(_menus[letterMenuItem].container.getBoundingClientRect().top + _menus[letterMenuItem].container.scrollHeight > _menuHeight ){
				_menus[letterMenuItem].container.style.top = String(_menuHeight - (_menus[letterMenuItem].container.getBoundingClientRect().top + _menus[letterMenuItem].container.scrollHeight))+"px";
			}
		}
		*/
...
SHoofddorp.MenuOld = (function () {
	...
	var _menuClick = function (that, id) {
		SHoofddorp.Menu.selectRoute(id);
		if (_selectedMenuElement) {
			if (Core.dom.hasClass(_selectedMenuElement, "initialSpecialButtonInnerMenuItem")) {
				Core.dom.removeClass(_selectedMenuElement, "initialTopButtonSelected");
				Core.dom.removeClass(_selectedMenuElement.parentNode.parentNode, "initialTopButtonSelected2");
